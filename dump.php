<?php
    ...
    function prepare_object_argument( $object, $depth = 1 ) {
        ...
        ob_start();
        echo $object_amount > 0 ? '<span style="cursor: pointer;" data-id="' . $html_id . '" onclick="clickableObjectHandler(this);">' . $class_name . ' ( ' . $object_amount . ' )' : $class_name . ' ( ' . $object_amount . ' )';
        echo $object_amount > 0 ? ' <em>{...}</em> {</span>' : ' <em>{}</em>';

        if ( $object_amount > 0 ) {
            echo '<span id="' . $html_id . '">';
        }
        ...
        foreach ( $object_to_check as $key => $value ) {
            ...
            echo PHP_EOL . str_repeat( DUMP_CODE_SPACE, $depth ) . $property_visibility . '[ ' . $key_additional . '<span style="color: #92008d;">' . $key . '</span>' . $key_additional . ' ] => ' . prepare_simple_argument( $value );

            if ( is_array( $value ) ) {
                echo prepare_array_argument( $value, $depth + 1 );
            }

            if ( is_object( $value ) ) {
                echo prepare_object_argument( $value, $depth + 1 );
            }
        }

        if ( $object_amount > 0 ) {
            echo PHP_EOL . str_repeat( DUMP_CODE_SPACE, $depth - 1 ) . '</span>}';
        }
    ...
    function prepare_array_argument( $array, $depth = 1 ) {
        ...
        echo $array_amount > 0 ? '<span style="cursor: pointer;" data-id="' . $html_id . '" onclick="clickableObjectHandler(this);">' : '';
        echo $array_amount > 0 ? '( ' . $array_amount . ' )' : '( ' . $array_amount . ' )';
        echo $array_amount > 0 ? ' <em>[...]</em> {</span>' : ' <em>[]</em>';
        ...
        foreach ( $array as $key => $value ) {
            ...
            echo PHP_EOL . str_repeat( DUMP_CODE_SPACE, $depth ) . '[ ' . $key_additional . '<span style="color: #92008d;">' . $key . '</span>' . $key_additional . ' ] => ' . prepare_simple_argument( $value );
    ...
    function prepare_simple_argument( $argument ) {
        ...
        switch ( gettype( $argument ) ) {
            case 'boolean':
                $color  = '#92008d';
                echo 'bool( <span style="color:' . $color . '">';
                echo $argument ? 'true' : 'false';
                echo '</span> )';
                break;
            case 'integer':
                $color  = '#ff3f38';
                echo 'int( <span style="color:' . $color . '">' . $argument . '</span> )';
                break;
            case 'double':
                $color  = '#14719a';
                echo 'float( <span style="color:' . $color . '">' . $argument . '</span> )';
                break;
            case 'string':
                $color  = '#1f6c53';
                echo 'string( ' . strlen( $argument ) . ' ) ' . '\'' . '<span style="color:' . $color . '">' . htmlspecialchars( $argument, ENT_QUOTES | ENT_IGNORE | ENT_HTML401 ) . '</span>' . '\'';
                break;
            case 'resource':
                $color  = '#6a149a';
                echo '{ <span style="color:' . $color . '">resource</span> }';
                break;
            case 'NULL':
                $color  = '#92008d';
            ...
            case 'unknown type':
                $color  = '#606060';
                echo '{ <span style="color:' . $color . '">unknown</span> }';
                break;
        }
...
CSS;
        ...
        foreach ( $arguments as $index => $argument ) {
            if ( is_array( $argument ) ) {
                echo prepare_array_argument( $argument );
            }
            else if ( is_object( $argument ) ) {
                echo prepare_object_argument( $argument );
            }
            else {
                echo prepare_simple_argument( $argument );
            }
...
JS;
        $js_min = <<<JS
        function clickableObjectHandler(e){var t=document.getElementById(e.getAttribute("data-id"));t.classList.contains("hidden")?t.classList.remove("hidden"):t.classList.add("hidden")}var thisDumpScript=document.querySelector('script[name="dump"]');thisDumpScript.parentNode.removeChild(thisDumpScript);
JS;
